...
DECLARE
    ...
    semantic_threshold FLOAT := 0.6; -- 高阈值，确保严格过滤
...
BEGIN
    ...
    FOREACH ing IN ARRAY forbidden_ingredients LOOP
        ...
        -- 检查菜名中是否包含忌口食材
        IF IN_菜名 ILIKE '%' || ing || '%' OR similarity(IN_菜名, ing) > 0.5 THEN
            RETURN FALSE; -- 找到匹配，不符合条件
        END IF;
        
        -- 检查菜系中是否包含忌口食材
        IF IN_菜系 ILIKE '%' || ing || '%' OR similarity(IN_菜系, ing) > 0.5 THEN
            RETURN FALSE; -- 找到匹配，不符合条件
        END IF;
        ...
        IF IN_食材分类 IS NOT NULL AND jsonb_typeof(IN_食材分类) = 'array' AND jsonb_array_length(IN_食材分类) > 0 THEN
            ...
            LOOP
                IF category_record.category ILIKE '%' || ing || '%' OR 
                   similarity(category_record.category, ing) > 0.5 THEN
                    RETURN FALSE; -- 找到匹配，不符合条件
                END IF;
            END LOOP;
        END IF;
...
DECLARE
    ...
    query_embedding vector(1536);
    similarity_score FLOAT;
    SEMANTIC_SEARCH_THRESHOLD FLOAT := 0.8; -- 语义搜索匹配的阈值
BEGIN
    -- 如果没有必选食材筛选条件，直接返回真
    IF NOT has_elements(required_ingredients) THEN
        RETURN TRUE;
    END IF;
    
    -- 使用AND逻辑：必须匹配所有必选食材才返回TRUE
    FOREACH ing IN ARRAY required_ingredients LOOP
        ...
        -- 2. 在菜名中匹配
        IF IN_菜名 ILIKE '%' || ing || '%' OR 
           similarity(IN_菜名, ing) > 0.4 THEN
            CONTINUE;
        END IF;
        
        -- 3. 在菜系中匹配
        IF IN_菜系 ILIKE '%' || ing || '%' OR 
           similarity(IN_菜系, ing) > 0.4 THEN
            CONTINUE;
        END IF;
        ...
        IF IN_食材分类 IS NOT NULL AND jsonb_typeof(IN_食材分类) = 'array' AND jsonb_array_length(IN_食材分类) > 0 THEN
            FOR ing_record IN
                SELECT 
                    jsonb_array_elements_text(IN_食材分类) AS category
            LOOP
                IF ing_record.category ILIKE '%' || ing || '%' OR 
                   similarity(ing_record.category, ing) > 0.4 THEN
                    -- 找到匹配，跳出内循环
                    CONTINUE;
                END IF;
            END LOOP;
...
RETURNS BOOLEAN AS $$
DECLARE
    semantic_threshold FLOAT := 0.5; -- 中等阈值，允许一定语义相似
    query_embedding vector(1536);
    embedding_record RECORD;
    category_record RECORD;
BEGIN
    ...
    IF IN_菜名 = search_query OR 
       ...
       similarity(IN_菜名, search_query) > 0.3 THEN
        RETURN TRUE;
    END IF;
    ...
    IF IN_菜系 = search_query OR 
       ...
       similarity(IN_菜系, search_query) > 0.3 THEN
        RETURN TRUE;
    END IF;
    ...
    IF IN_食材分类 IS NOT NULL AND jsonb_typeof(IN_食材分类) = 'array' AND jsonb_array_length(IN_食材分类) > 0 THEN
        ...
        LOOP
            IF category_record.category = search_query OR 
               ...
               similarity(category_record.category, search_query) > 0.3 THEN
                RETURN TRUE;
            END IF;
        END LOOP;
    END IF;
